...
// Función para generar un problema de potencias o raíces con resultado entero
function generatePowersAndRootsProblem() {
let base, exponent, isRootProblem, result;

do {
base = Math.floor(Math.random() * 16) + 1;
exponent = Math.floor(Math.random() * 10) + 1;
isRootProblem = Math.random() < 0.5; // 50% de probabilidad de generar un problema de raíces

if (isRootProblem) {
    // Asegura que la raíz cuadrada tenga un resultado entero
    result = Math.sqrt(base);
} else {
    // Asegura que el resultado de la potencia sea un número entero
    result = Math.pow(base, exponent);
}
} while (!Number.isInteger(result)); // Repetir hasta obtener un número entero

const problemContainer = document.getElementById('problem-container');
const solutionContainer = document.getElementById('solution-container');

if (isRootProblem) {
problemContainer.innerHTML = `Calcula √${base}`;
solutionContainer.innerHTML = `<div class="solution">√${base} es ${result}</div>`;
} else {
problemContainer.innerHTML = `Calcula ${base}<sup>${exponent}</sup>`;
solutionContainer.innerHTML = `<div class="solution">${base}<sup>${exponent}</sup> es ${result}</div>`;
}
...
// Función para generar un problema de expresiones algebraicas
...
function generate3DGeometryProblem() {
    ...
    switch (selectedShape) {
        case 'cubo':
            const sideLength = Math.floor(Math.random() * 60) + 1;
        ...
        case 'cilindro':
            const cylinderRadius = Math.floor(Math.random() * 60) + 1;
            const cylinderHeight = Math.floor(Math.random() * 60) + 1;
        ...
        case 'prisma':
            const prismBase = Math.floor(Math.random() * 60) + 1;
            const prismHeight = Math.floor(Math.random() * 60) + 1;
